angular.module('phoenix.controllers', [])
  ...
  .controller('MapCtrl', function ($scope, $ionicLoading, $q,$stat, $cordovaGeolocation, GoogleMaps, $cordovaNetwork, $ionDrawerVerticalDelegate, $ionicSlideBoxDelegate, $ionicPlatform, ConnectivityMonitor, DataService, Marker, AuthService, localStorageService) {
    $state.reload();
    $scope.username = AuthService.getCurrentEmail();
    if ($scope.username === 'undefined' || $scope.username === null)
      $state.go('app.login');

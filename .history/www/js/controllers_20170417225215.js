angular.module('phoenix.controllers', [])
  ...
  .controller('MapCtrl', function ($scope, $ionicLoading, $q, $state,$route, $cordovaGeolocation, GoogleMaps, $cordovaNetwork, $ionDrawerVerticalDelegate, $ionicSlideBoxDelegate, $ionicPlatform, ConnectivityMonitor, DataService, Marker, AuthService, localStorageService) {
    $route.reload();
    $scope.username = AuthService.getCurrentEmail();
    if ($scope.username === 'undefined' || $scope.username === null)
      $state.go('app.login');

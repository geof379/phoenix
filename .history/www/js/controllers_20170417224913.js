angular.module('phoenix.controllers', [])
  ...
  .controller('MapCtrl', function ($scope, $ionicLoading, $q, $state, $cordovaGeolocation, GoogleMaps, $cordovaNetwork, $ionDrawerVerticalDelegate, $ionicSlideBoxDelegate, $ionicPlatform, ConnectivityMonitor, DataService, Marker, AuthService, localStorageService) {
    $state.reload();
    $scope.username = AuthService.getCurrentEmail();
    if ($scope.username === 'undefined' || $scope.username === null)
      $state.go('app.login');
          ...
          function (position) {
            ...
            angular.forEach(oldSearchList, function (object, key) {
              ...
              if (GoogleMaps.getDistanceBetweenPoints(position, shopMarker) > localStorageService.get('distance')) {
                if ($scope.temp.searchlists.indexOf(object) < 0)
                  $scope.temp.searchlists.push(object);
              }
            })
            $scope.searchlists = [];
            $scope.searchlists = $scope.temp.searchlists
          }
        );
